...
设计时先锁定机场到服务处的入口视线，再划分住宅、商业、自然和活动区域。预留连续步道、建筑占地与操作空间；`pathDirt`、`pathSand`、`pathStone`、`pathBrick` 必须完全位于 `level1` 陆地内，遇到河流或湖泊时在岸边断开，仅桥梁可以跨越水面。桥梁应跨越有效河道，坡道只连接相邻高度，避免无法步行到达的孤岛或高地。交付新岛屿前必须运行设计校验，确认 JSON 可解析、所有地板位于陆地、固定对象坐标未变、河口与海岸未变，并在导出图中检查通行性和设施重叠。
